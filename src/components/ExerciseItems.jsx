...
const ExerciseItems = () => {
  ...
  const renderWorkoutItem = ({ item }) => (
    <TouchableOpacity onPress={() => navigation.navigate("Exercise", { item })}>
      <ImageBackground source={exerciseImage} className="h-44 w-40 rounded-2xl overflow-hidden my-2">
  ...
  return (
    <View className="min-h-screen">
      <FlashList data={workoutData} renderItem={RenderRow} keyExtractor={(item) => item.id} showsVerticalScrollIndicator={false} estimatedItemSize={200} />
    </View>
  );
};
